<div class="w-full max-w-lg mx-auto mt-5 md:ml-8 md:mt-0 md:w-1/2">
    ...
    <div class="mt-2">
        ...
        <div class="flex items-center mt-1 mx-4">
            <span class="text-gray-600 text-md">{{ $article->price }} Euros</span>
        </div>
    </div>
    <div class="mt-2">
        <label class="text-gray-700 text-xl" for="disponibility">Disponibilité</label>
        ...
        @if (auth()->user()->role == 2)
            @if ($countDateDispo != 0)
                <button wire:click="confirmingRent()"
                    class="px-8 py-2 bg-red-500 text-white text-lg font-medium rounded hover:bg-red-600 focus:outline-none focus:bg-red-600">Louer
                    Maintenant</button>
            @else
                <button
                    class="px-8 py-2 bg-red-300 text-white text-lg font-medium rounded focus:outline-none cursor-not-allowed">Pas
                    disponible</button>
            @endif
