            ...
            @forelse($notifications as $notification)
                <div class="flex items-center px-4 py-3 border-b hover:bg-gray-100 -mx-2">
                    <img class="h-8 w-8 rounded-full object-cover mx-1"
                        src="{{ $notification->demande->annoncedispo->annonce->image1 }}" alt="avatar">
                    <p class="text-gray-600 text-sm mx-2">
                        <a class="font-bold" href="/article/{{ $notification->demande->annoncedispo->annonce->id }}">
                            {{ $notification->demande->annoncedispo->annonce->car_model }}</a>
                        {{ $notification->message }}
                    </p>
                </div>
            @empty
                <div class="flex items-center px-4 py-3 border-b hover:bg-gray-100 -mx-2">
                    <p class="uppercase text-red-500 text-center"> Aucune notification </p>
                </div>
            @endforelse
